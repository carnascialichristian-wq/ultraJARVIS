...
def _read_one() -> Optional[Union[str, dict]]:
    """
    Pop the first non-empty line from the queue.

    Returns:
        - dict on success
        - "__EMPTY__" / "__BAD__" for skippable lines
...
def process_one() -> Optional[dict]:
    """
    Process a single job from the queue (if any).

    Returns the result record that was written to done.jsonl, or None.
    """
...
def run_forever(sleep_sec: float = 3.0) -> None:
    """Main worker loop – keeps draining the queue."""
    while True:
        result = process_one()
        if result is None:
...
def queue_status() -> dict:
    """Return a quick status of the queue / done files."""
    pending = 0
    if QUEUE.exists():
